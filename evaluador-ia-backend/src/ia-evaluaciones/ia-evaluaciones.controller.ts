...
import { EvaluacionGenerada } from './interfaces/evaluacion.interface'; // ✅ NUEVA IMPORTACIÓN
...
export class IaEvaluacionesController {
  ...
  @UseInterceptors(
    FileInterceptor('pdf', {
      storage: memoryStorage(), // 👈 Se guarda el archivo en memoria temporal
      limits: {
  ...
  async generarEvaluacion(
    ...
    try {
      const evaluacionGenerada = await this.iaService.generarEvaluacion(
        file.buffer,
        moduloId,
      );
